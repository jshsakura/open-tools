...
import { Upload, Music, Download, Loader2, X, Scissors, FileAudio } from "lucide-react"
...
export function AudioTrimmer() {
    ...
    const process = async () => {
        ...
        try {
            ...
            const args = buildTrimArgs({ inputName, outputName, start, end })
            const blob = await run(inputName, file, args, outputName, mime)
                    ...
                    <div className="p-6 flex-1 relative space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <Input
                                        ...
                                        disabled={!file}
                                    />
                                    <p className="text-xs text-muted-foreground">{t("startLabel")}</p>
                                </div>
                                <div className="space-y-1.5">
                                    <Input
                                        type="number"
                                        ...
                                        disabled={!file}
                                    />
                                    <p className="text-xs text-muted-foreground">{t("endLabel")}</p>
                                </div>
                            </div>
                        ...
                        <div className="mt-auto pt-4">
                            <Button
                                ...
                                onClick={process}
                                disabled={!file || !loaded || processing || end <= start}
                            >
                                {processing ? (
