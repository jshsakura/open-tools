...
const OUTPUT_NAME = "boomerang.mp4"
const OUTPUT_MIME = "video/mp4"

const LOOP_OPTIONS = Array.from(
    { length: MAX_LOOPS - MIN_LOOPS + 1 },
    (_, i) => MIN_LOOPS + i,
)

export function VideoBoomerang() {
    const t = useTranslations("VideoBoomerang")
    ...
    const [loops, setLoops] = useState(MIN_LOOPS)
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            console.error("Video boomerang error:", err)
            toast.error(t("error"))
        } finally {
            setProcessing(false)
        }
            ...
            <div className="w-full lg:w-1/2">
                <GlassCard className="h-full rounded-2xl p-6 flex flex-col gap-8 relative">
                    ...
                    <div className="space-y-3">
                        <Label className="text-base">{t("loopsLabel")}</Label>
                        <p className="text-xs text-muted-foreground">{t("loopsHint")}</p>
                        <div className="grid grid-cols-5 gap-2">
                            {LOOP_OPTIONS.map((option) => (
                                <button
                                    key={option}
                                    type="button"
                                    onClick={() => setLoops(option)}
                                    className={cn(
                                        "rounded-xl border-2 py-3 font-bold text-lg transition-all",
                                        loops === option
