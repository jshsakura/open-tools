...
import { Upload, Music, Download, Loader2, X, SlidersHorizontal, FileAudio } from "lucide-react"
...
import { buildEqArgs, MIN_GAIN, MAX_GAIN } from "./audio-equalizer.utils"

export function AudioEqualizer() {
    const t = useTranslations("AudioEqualizer")
    ...
    const process = async () => {
        ...
        try {
            ...
            const outputName = `eq.${spec.ext}`
            const args = buildEqArgs({ inputName, outputName, bass, treble, format, bitrate })
            ...
            setResultUrl(url)
            setResultName(`${baseName}-eq.${spec.ext}`)
            toast.success(t("success"))
        } catch (err) {
            console.error("Audio equalizer error:", err)
            toast.error(t("error"))
        } finally {
            setProcessing(false)
        }
                    ...
                    <div className="p-6 flex-1 relative space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <Slider
                                ...
                                max={MAX_GAIN}
                                step={1}
                                onValueChange={([v]) => setBass(Math.round(v))}
                                className="py-2"
                            />
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <Slider
                                ...
                                max={MAX_GAIN}
                                step={1}
                                onValueChange={([v]) => setTreble(Math.round(v))}
                                className="py-2"
                            />
                        </div>

                        <p className="text-xs text-muted-foreground">{t("eqHint")}</p>
