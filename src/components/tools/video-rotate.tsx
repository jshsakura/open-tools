...
import { buildRotateArgs, type Rotation } from "./video-rotate.utils"

const OUTPUT_NAME = "rotated.mp4"
const OUTPUT_MIME = "video/mp4"

const ROTATION_OPTIONS: Rotation[] = [0, 90, 180, 270]

export function VideoRotate() {
    const t = useTranslations("VideoRotate")
    ...
    const handleRotate = async () => {
        ...
        try {
            ...
            console.error("Video rotate error:", err)
            toast.error(t("error"))
        } finally {
            setProcessing(false)
        }
            ...
            <div className="w-full lg:w-1/2">
                <GlassCard className="h-full rounded-2xl p-6 flex flex-col gap-8 relative">
                    ...
                    <div className="space-y-3">
                        <Label className="text-base">{t("rotationLabel")}</Label>
                        <Select
                            value={String(rotation)}
                            onValueChange={(value) => setRotation(Number(value) as Rotation)}
                        >
                    ...
                    {/* Flip toggles */}
                    <div className="space-y-3">
                        <Label className="text-base">{t("flipLabel")}</Label>
