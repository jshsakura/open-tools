...
import { Upload, Music, Download, Loader2, X, Rewind, FileAudio } from "lucide-react"
...
export function AudioReverse() {
    const t = useTranslations("AudioReverse")
    ...
    const process = async () => {
        ...
        try {
            ...
            const outputName = `reversed.${spec.ext}`
            const args = buildReverseArgs({ inputName, outputName, format, bitrate })
            ...
            setResultUrl(url)
            setResultName(`${baseName}-reversed.${spec.ext}`)
            toast.success(t("success"))
        } catch (err) {
            console.error("Audio reverse error:", err)
            toast.error(t("error"))
        } finally {
            setProcessing(false)
        }
            ...
            <div className="w-full lg:w-1/2">
                <GlassCard className="h-full rounded-2xl overflow-hidden flex flex-col">
                    <div className="p-6 flex-1 relative space-y-6">
                        ...
                        <p className="text-sm text-muted-foreground">{t("infoNote")}</p>
