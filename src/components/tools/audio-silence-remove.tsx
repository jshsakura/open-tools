...
import { Upload, Music, Download, Loader2, X, VolumeX, FileAudio } from "lucide-react"
...
import { buildSilenceArgs, type SilenceMode, DEFAULT_THRESHOLD } from "./audio-silence-remove.utils"

const MODES: SilenceMode[] = ["leading", "trailing", "both"]

export function AudioSilenceRemove() {
    const t = useTranslations("AudioSilenceRemove")
    ...
    const [mode, setMode] = useState<SilenceMode>("both")
    ...
    const process = async () => {
        ...
        try {
            ...
            const outputName = `trimmed.${spec.ext}`
            const args = buildSilenceArgs({ inputName, outputName, mode, threshold, format, bitrate })
            ...
            setResultUrl(url)
            setResultName(`${baseName}-nosilence.${spec.ext}`)
            toast.success(t("success"))
        } catch (err) {
            console.error("Audio silence remove error:", err)
            toast.error(t("error"))
        } finally {
            setProcessing(false)
        }
                    ...
                    <div className="p-6 flex-1 relative space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <Label className="text-base">{t("thresholdLabel")}</Label>
                                <span className="font-mono font-bold text-primary text-lg">{threshold} dB</span>
                            </div>
