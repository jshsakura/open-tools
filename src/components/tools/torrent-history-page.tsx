...
import { AlertTriangle, Fingerprint, ShieldAlert, CheckCircle, Globe, Lock, Info, Server, Clock, MapPin, Activity } from "lucide-react"
...
type TorrentFetchPayload = TorrentResult & {
  debugLog?: string[]
  error?: string
}

export function TorrentHistoryPageClient() {
  const t = useTranslations("TorrentHistory")
  const tool = getToolById('torrent-history')
  const [status, setStatus] = useState<'idle' | 'scanning' | 'result'>('idle')
  const [progress, setProgress] = useState(0)
  const [logs, setLogs] = useState<string[]>([])
  const [data, setData] = useState<TorrentResult | null>(null)
  ...
  const startScan = async () => {
    if (data?.ip) {
      ...
      if (cached) {
        const parsed = JSON.parse(cached) as { timestamp: number; data: TorrentResult }
    ...
    const fetchPromise = fetch('/api/torrent-history')
      .then(async res => {
        const payload = (await res.json()) as TorrentResult & { debugLog?: string[]; error?: string }
    ...
    let fetchOk = false
    let fetchData: TorrentFetchPayload | null = null
    let fetchError: unknown = null

    fetchPromise.then(result => {
      fetchOk = result.ok
      if ('data' in result) fetchData = result.data ?? null
      if ('error' in result) fetchError = result.error ?? null
      isDataReady = true
    })
    ...
    if (!isDataReady) {
      setLogs(prev => [...prev, t("scanning.logs.7")])
      while (!isDataReady) {
    ...
    try {
      const resolvedData: TorrentFetchPayload | null = fetchData

      if (!fetchOk || !resolvedData) {
        const message = fetchError instanceof Error ? fetchError.message : "Failed to fetch history"
        throw new Error(message)
      }

      const resultData: TorrentFetchPayload = resolvedData
      if (resultData.error === "BLOCKED") {
        setError("BLOCKED")
        setData(resultData)
        setStatus('result')
        return
      ...
      const message = err instanceof Error ? err.message : "Failed to retrieve data"
  ...
  useEffect(() => {
    const resolveIp = async () => {
      try {
        ...
        const payload = (await res.json()) as { ip?: string }
    ...
    <div className="container mx-auto px-4 py-12 max-w-6xl"><div className="mb-12 space-y-4">
      ...
      <div className="grid gap-8">
        <Card className="border-none bg-transparent shadow-none overflow-visible relative min-h-[400px]">
          ...
          <CardContent className="p-6 md:px-8 md:pb-6 md:pt-4 flex flex-col items-center min-h-[400px] relative z-20 bg-transparent">
            <AnimatePresence mode="wait">
              {status === 'idle' && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.1, filter: "blur(10px)" }} className="flex flex-col items-center gap-12 w-full">
                  <button
                  ...
                  >
                    ...
                    <div className="absolute -bottom-20 flex flex-col items-center gap-3 w-full text-center">
                      <div className="font-mono text-sm text-blue-600/80 dark:text-orange-500/80 tracking-[0.2em] uppercase animate-pulse">{t("analysis.targetDetected")}</div>
              ...
              {status === 'scanning' && (
                <motion.div key="scanning" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-lg space-y-10">
                  <div className="relative w-40 h-40 mx-auto">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center font-mono text-4xl font-black text-primary drop-shadow-lg">{progress}%</div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="font-mono text-sm space-y-3 border border-blue-500/10 dark:!border-stone-800/30 bg-transparent dark:!bg-stone-900/20 p-6 rounded-lg relative">
                      ...
                      {logs.map((log) => (
                        <motion.div key={log} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3 text-muted-foreground">
              ...
              {status === 'result' && data && (
                <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full space-y-8">
                  ...
                  {isVpnDetected(data.isp || '') && (
                    <Alert className="bg-blue-500/10 border-blue-500/20 text-blue-600 dark:text-blue-400">
                      ...
                      <AlertTitle className="text-blue-700 dark:text-blue-300 font-bold ml-2">{t("analysis.vpnDetectedTitle")}</AlertTitle>
                      <AlertDescription className="ml-2 mt-1 opacity-90">{t("results.vpnDetected")}</AlertDescription>
                    </Alert>
                  )}
                  ...
                  {error === "BLOCKED" && (
                    <div className="text-center py-16 border border-orange-500/30 rounded-2xl bg-orange-500/5 space-y-6 relative overflow-hidden">
                      ...
                      <div className="relative z-10">
                        ...
                        <p className="text-muted-foreground max-w-md mx-auto leading-relaxed break-keep">{t("results.blockedDesc")}</p>
                      </div>
                      <Button asChild className="relative z-10 bg-orange-600 hover:bg-orange-500 text-white font-bold h-12 px-8">
                        <a href="https://iknowwhatyoudownload.com/" target="_blank" rel="noopener noreferrer">{t("results.manualCheck")}</a>
                      </Button>
                    </div>
                  )}
                      ...
                      {data.downloads && data.downloads.length > 0 ? (
                            ...
                            <table className="w-full text-sm text-left">
                              ...
                              <tbody className="divide-y divide-border">
                                {data.downloads.map((item) => (
                                  <tr key={`${item.title}-${item.date}`} className={`transition-colors ${item.isSensitive ? 'bg-red-500/5 hover:bg-red-500/10' : 'hover:bg-blue-500/5'}`}>
                                      ...
                                      <div className="flex items-start gap-4">
                                        ...
                                        <div>
                                          ...
                                          <div className="flex items-center gap-2 mt-2">
                                            <span className="text-xs px-2 py-0.5 rounded bg-blue-500/5 text-muted-foreground border border-border font-mono">{item.size || 'N/A'}</span>
                                            <span className="text-xs text-muted-foreground uppercase font-bold tracking-wider">{item.category || 'Unknown'}</span>
                                          </div>
                                        </div>
                                      </div>
                                    </td>
                        ...
                        <div className="text-center py-20 border border-dashed border-green-500/30 rounded-2xl bg-green-500/5 space-y-6">
                          ...
                          <div>
                            ...
                            <p className="text-muted-foreground max-w-md mx-auto break-keep">{t("results.cleanDesc")}</p>
                          </div>
                          <div className="inline-block">
                            <span className="text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-500/10 px-4 py-2 rounded-full border border-amber-500/30">{t("results.cleanWarning")}</span>
                          </div>
                        </div>
                      )}
                    </>
          ...
          <div className="p-5 sm:p-6 md:px-8 md:py-4 border-t border-border/10 dark:!border-stone-800/30 bg-transparent relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-b from-transparent to-background/5 pointer-events-none" />
            <div className="grid gap-6">
              <div className="bg-transparent dark:!bg-stone-950/30 backdrop-blur-md border border-slate-200 dark:!border-stone-800 rounded-2xl p-5 sm:p-6 md:p-6 space-y-6 text-left shadow-sm">
                ...
                <div className="grid md:grid-cols-2 gap-8 text-sm text-muted-foreground break-keep">
                    ...
                    <ul className="space-y-2">
                      {['0', '1', '2'].map((itemKey) => (
                        <li key={itemKey} className="flex items-start gap-2">
                          <span className="text-blue-600 dark:!text-orange-500 mt-1">✓</span> {t(`guide.whyVpnList.${itemKey}`)}
                        </li>
                      ))}
                    </ul>
                  </div>
                  <div className="space-y-3">
                    ...
                    <p className="leading-relaxed break-keep">{t("guide.policyDesc")}</p>
                  </div>
                </div>
              </div>
              ...
              <Alert className="bg-amber-50/50 dark:bg-amber-950/10 border border-amber-200 dark:border-amber-900/50 rounded-xl text-amber-900 dark:text-amber-500">
                ...
                <AlertDescription className="ml-2 mt-1 text-xs opacity-90 leading-relaxed break-keep">{t("disclaimer.content")}</AlertDescription>
              </Alert>
            </div>
          </div>
        </Card>
