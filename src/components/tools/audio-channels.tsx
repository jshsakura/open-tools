...
import { Upload, Music, Download, Loader2, X, AudioLines, FileAudio } from "lucide-react"
...
import { buildChannelsArgs, type ChannelMode } from "./audio-channels.utils"

export function AudioChannels() {
    const t = useTranslations("AudioChannels")
    ...
    const process = async () => {
        ...
        try {
            ...
            const outputName = `${mode}.${spec.ext}`
            const args = buildChannelsArgs({ inputName, outputName, mode, format, bitrate })
            ...
            setResultUrl(url)
            setResultName(`${baseName}-${mode}.${spec.ext}`)
            toast.success(t("success"))
        } catch (err) {
            console.error("Audio channels error:", err)
            toast.error(t("error"))
        } finally {
            setProcessing(false)
        }
                    ...
                    <div className="p-6 flex-1 relative space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                >
                                    <span className="font-semibold text-sm">{t("modeMono")}</span>
                                </div>
                                <div
                                ...
                                >
                                    <span className="font-semibold text-sm">{t("modeStereo")}</span>
                                </div>
                            </div>
