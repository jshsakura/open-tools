...
import { Upload, Music, Download, Loader2, X, Gauge, FileAudio } from "lucide-react"
...
import { buildSpeedArgs, MIN_SPEED, MAX_SPEED } from "./audio-speed.utils"

const SPEED_STEP = 0.05

export function AudioSpeed() {
    const t = useTranslations("AudioSpeed")
    ...
    const process = async () => {
        ...
        try {
            ...
            const outputName = `speed.${spec.ext}`
            const args = buildSpeedArgs({ inputName, outputName, speed, format, bitrate })
            ...
            setResultUrl(url)
            setResultName(`${baseName}-${speed}x.${spec.ext}`)
            toast.success(t("success"))
        } catch (err) {
            console.error("Audio speed error:", err)
            toast.error(t("error"))
        } finally {
            setProcessing(false)
        }
                    ...
                    <div className="p-6 flex-1 relative space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="font-mono font-bold text-primary text-lg">{speed}x</span>
                            </div>
