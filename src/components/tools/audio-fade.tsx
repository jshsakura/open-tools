...
import { Upload, Music, Download, Loader2, X, Waves, FileAudio } from "lucide-react"
...
import { buildFadeArgs } from "./audio-fade.utils"

export function AudioFade() {
    const t = useTranslations("AudioFade")
    ...
    const process = async () => {
        ...
        try {
            ...
            const outputName = `fade.${spec.ext}`
            const args = buildFadeArgs({ inputName, outputName, duration, fadeIn, fadeOut, format, bitrate })
            ...
            setResultUrl(url)
            setResultName(`${baseName}-fade.${spec.ext}`)
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <Input
                                        ...
                                        disabled={!file}
                                    />
                                    <p className="text-xs text-muted-foreground">{t("fadeInLabel")}</p>
                                </div>
                                <div className="space-y-2">
                                    <Input
                                        type="number"
                                        ...
                                        disabled={!file}
                                    />
                                    <p className="text-xs text-muted-foreground">{t("fadeOutLabel")}</p>
                                </div>
                            </div>
                            <p className="text-xs text-muted-foreground">{t("fadeHint")}</p>
                        </div>
