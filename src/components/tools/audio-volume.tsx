...
import { Upload, Music, Download, Loader2, X, Volume2, FileAudio } from "lucide-react"
...
import { buildVolumeArgs, type VolumeMode } from "./audio-volume.utils"

export function AudioVolume() {
    const t = useTranslations("AudioVolume")
    ...
    const [mode, setMode] = useState<VolumeMode>("gain")
    ...
    const process = async () => {
        ...
        try {
            ...
            const outputName = `volume.${spec.ext}`
            const args = buildVolumeArgs({ inputName, outputName, mode, gainDb, format, bitrate })
            ...
            setResultUrl(url)
            setResultName(`${baseName}-volume.${spec.ext}`)
            toast.success(t("success"))
        } catch (err) {
            console.error("Audio volume error:", err)
            toast.error(t("error"))
        } finally {
            setProcessing(false)
        }
                    ...
                    <div className="p-6 flex-1 relative space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                >
                                    <span className="font-bold text-sm">{t("modeGain")}</span>
                                </div>
                                <div
                                ...
                                >
                                    <span className="font-bold text-sm">{t("modeNormalize")}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {mode === "gain" ? (
                            <div className="space-y-4">
                                ...
                                <Slider
                                    ...
                                    className="py-2"
                                />
                                <p className="text-xs text-muted-foreground">{t("gainHint")}</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                <p className="text-sm text-muted-foreground">{t("normalizeHint")}</p>
                            </div>
                        )}
