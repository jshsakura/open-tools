...
import { PromptOptimizer } from "@/components/tools/prompt-optimizer"
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return createToolMetadata({
    ...
    description: t("PromptOptimizer.description"),
...
export default async function PromptOptimizerPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  setRequestLocale(locale)

  const tool = getToolById("prompt-optimizer")
  const t = await getTranslations({ locale, namespace: "Catalog" })
  const ui = await getTranslations({ locale, namespace: "PromptOptimizer" })
