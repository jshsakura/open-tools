...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return createToolMetadata({
    locale,
    title: t("JsonSchemaGenerator.title"),
    description: t("JsonSchemaGenerator.description"),
...
export default async function JsonSchemaGeneratorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  setRequestLocale(locale)

  const tool = getToolById("json-schema-generator")
  const t = await getTranslations({ locale, namespace: "Catalog" })
  const ui = await getTranslations({ locale, namespace: "JsonSchemaGenerator" })
