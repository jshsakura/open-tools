...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return createToolMetadata({
    locale,
    title: t("PasswordStrength.title"),
    description: t("PasswordStrength.description"),
    path: "/tools/password-strength",
  })
}

export default async function PasswordStrengthPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: "PasswordStrength" })
