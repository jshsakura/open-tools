...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return createToolMetadata({
    locale,
    title: t("FontPairer.title"),
    description: t("FontPairer.description"),
...
export default async function FontPairerPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  setRequestLocale(locale)

  const tool = getToolById("font-pairer")
  const t = await getTranslations({ locale, namespace: "Catalog" })
  const ui = await getTranslations({ locale, namespace: "FontPairer" })
