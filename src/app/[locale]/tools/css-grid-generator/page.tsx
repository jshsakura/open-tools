...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return createToolMetadata({
    locale,
    title: t("CssGridGenerator.title"),
    description: t("CssGridGenerator.description"),
...
export default async function CssGridGeneratorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  setRequestLocale(locale)

  const tool = getToolById("css-grid-generator")
  const t = await getTranslations({ locale, namespace: "Catalog" })
  const ui = await getTranslations({ locale, namespace: "CssGridGenerator" })
