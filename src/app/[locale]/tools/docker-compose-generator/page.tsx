...
import { DockerComposeGenerator } from "@/components/tools/docker-compose-generator"
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return createToolMetadata({
    locale,
    title: t("DockerComposeGenerator.title"),
    description: t("DockerComposeGenerator.description"),
...
export default async function DockerComposeGeneratorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  setRequestLocale(locale)

  const tool = getToolById("docker-compose-generator")
  const t = await getTranslations({ locale, namespace: "Catalog" })
  const ui = await getTranslations({ locale, namespace: "DockerComposeGenerator" })
